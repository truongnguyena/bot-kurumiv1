...
var msg = r(á.message);
const res = await axios.get(`https://api.popcat.xyz/weather?q=${encodeURI('Đồng Tháp')}`);
...
msg = msg.replace(/{seconds}/g, seconds)
  msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://api-ryo.ducryovn.repl.co/api/hearing.php`)).data.url)
          msg = {
              body: msg, attachment: (await axios.get((await axios.get(`https://videosatria.ghuy0707.repl.co/videosatria`)).data.data, {
                  responseType: 'stream'
              })).data
          };
