...
module.exports.onLoad = async () => {
    ...
    const downloads = [
        ["data.json", "https://raw.githubusercontent.com/GinzaTech/Zwar/main/data.json"],
        ["items.json", "https://raw.githubusercontent.com/GinzaTech/Zwar/main/items.json"]
    ];
    for (const [file, url] of downloads) {
        if (fs.existsSync(dirMaterial + file)) continue;
        try {
            (await axios({ url, method: 'GET', responseType: 'stream' })).data.pipe(fs.createWriteStream(dirMaterial + file));
        } catch (e) {}
    }
