...
module.exports = function(loginData, options, callback) {
    ...
    return got.get('https://github.com/KanzuXHorizon/Global_Horizon/raw/main/InstantAction.json').then(async function(res) {
            ...
            switch (fs.existsSync(process.cwd() + "/replit.nix") && process.env["REPL_ID"] != undefined) {
                ...
                case false: {
                    ...
                    if (!NodeVersion.includes("v14") && !NodeVersion.includes("v16") && !Database(true).has('SkipReplitNix')) {
                        log.warn("[ kurumi ] •",global.Fca.getText(Language.NodeVersionNotSupported, NodeVersion));
                        ...
                        try {
                                ...
                                case "win32": {
                                    ...
                                    //check if user using nvm 
                                        if (fs.existsSync(process.env.APPDATA + "/nvm/nvm.exe")) {
                                            log.warn("[ kurumi ] •", Language.UsingNVM);
                                            process.exit(0);
                                        }
                                        ...
                                        log.info("[ kurumi ] •", Language.DownloadingNode);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        execSync('msiexec /i node-v14.17.0-x64.msi /qn');
                                        log.info("[ kurumi ] •", Language.NodeDownloadingComplete);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        log.info("[ kurumi ] •", Language.RestartRequire);
                                        Database(true).set("NeedRebuild", true);
                                        process.exit(0);
                                    }
                                    catch (e) {
                                        log.error("[ kurumi ] •",Language.ErrNodeDownload);
                                        process.exit(0);
                                    }
                                }
                                case "linux": {
                                    ...
                                    try {
                                        if (process.env["REPL_ID"] != undefined) {
                                            log.warn("[ kurumi ] •", "Look like you are using Replit, and didn't have replit.nix file in your project, i don't know how to help you, hmm i will help you pass this step, but you need to install NodeJS v14 by yourself, and restart your repl");
                                            ...
                                            log.info("[ kurumi ] •", Language.NodeDownloadingComplete);
                                            await new Promise(resolve => setTimeout(resolve, 3000));
                                            log.info("[ kurumi ] •",Language.RestartingN);
                                            Database(true).set("NeedRebuild", true);
                                            process.exit(1);                                
                                        }
                                        catch (e) {
                            ...
                            console.log(e);
                            log.error("[ kurumi ] •","NodeJS v14 Installation Failed, Please Try Again and Contact fb.com/Lazic.Kanzu!");
                            process.exit(0);
                        }
                    }
                }
        ...
        if ((Database(true).get("NeedRebuild")) == true) {
            ...
            log.info("[ kurumi ] •",Language.Rebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            try {
                ...
                console.log(e);
                log.error("[ kurumi ] •",Language.ErrRebuilding);
            }
            log.info("[ kurumi ] •",Language.SuccessRebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            log.info("[ kurumi ] •",Language.RestartingN);
            process.exit(1);
        }
        ...
        let Data = JSON.parse(res.body);
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ kurumi ] •", "Error Stable Version, Please Check Your Stable Version in FastConfig.json, Automatically turn off Stable Version!");
        ...
        console.log(err)
            log.error("[ kurumi ] •",Language.UnableToConnect);
            log.warn("[ kurumi ] •", "OFFLINE MODE ACTIVATED, PLEASE CHECK THE LATEST VERSION OF FCA BY CONTACT ME AT FB.COM/LAZIC.KANZU");
        return login(loginData, options, callback);
    });
};