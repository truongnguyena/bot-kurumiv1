                ...
                if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
                    ...
                    if (delta.deltaMessageReply.repliedToMessage) {
                        ...
                        return defaultFuncs
                            ...
                            .then((resData) => {
                                resData = utils.normalizeGraphQLBatch(resData);
                                const meta = utils.getGraphQLBatchMeta(resData);
                                if (meta.error_results > 0) throw resData[0].o0?.errors || resData[0];
                                if (meta.successful_results === 0) throw { error: "forcedFetch: there was no successful_results", res: resData };
...
module.exports = function (defaultFuncs, api, ctx) {
    ...
    getSeqID = function getSeqID(useFallbackDoc) {
        ctx.t_mqttCalled = false;
        const seqForm = useFallbackDoc ? {
            "av": ctx.globalOptions.pageID,
            "queries": JSON.stringify({
                "o0": {
        ...
        defaultFuncs
            .post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, seqForm)
            .then(utils.parseAndCheckLogin(ctx, defaultFuncs))
            .then((resData) => {
                resData = utils.normalizeGraphQLBatch(resData);
                const meta = utils.getGraphQLBatchMeta(resData);
                if (utils.getType(resData) != "Array") {
                    log.error("getSeqId", "Invalid GraphQL response format", resData);
                    if (ctx.lastSeqId) return listenMqtt(defaultFuncs, api, ctx, globalCallback);
                    if (!useFallbackDoc) return getSeqID(true);
                    return globalCallback({ error: "getSeqId: invalid GraphQL response", res: resData });
                }
                if (meta.error_results > 0) throw resData[0].o0?.errors || resData[0];
                if (meta.successful_results === 0) throw { error: "getSeqId: there was no successful_results", res: resData };
                let syncSequenceId = utils.extractSyncSequenceId(resData);
                if (!syncSequenceId) {
                    try {
                        syncSequenceId = resData[0].o0.data.viewer.message_threads.sync_sequence_id;
                    } catch (e) {}
                }
                if (syncSequenceId) {
                    ctx.lastSeqId = syncSequenceId;
