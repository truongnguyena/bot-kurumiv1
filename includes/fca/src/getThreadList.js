...
module.exports = function (defaultFuncs, api, ctx) {
  return function getThreadList(limit, timestamp, tags, callback) {
    ...
    defaultFuncs
      ...
      .then((resData) => {
        if (!resData) throw new Error("Empty GraphQL response");
        ...
        if (meta.successful_results === 0) throw { error: "getThreadList: there was no successful_results", res: resData };

        const threads = resData[0]?.o0?.data?.viewer?.message_threads;
        if (!threads || !threads.nodes) throw { error: "getThreadList: invalid payload", res: resData };

        if (timestamp) threads.nodes.shift();

        callback(null, formatThreadList(threads.nodes));
      })
      .catch((err) => {
