...
function requireDependency(name, fallbackPath) {
  if (name === 'canvas') {
    try {
...
const langData = langFile.filter(item => item.indexOf('kurumi') != 0 && item != '');
...
if (global.config.version != '20.0.0') {
              ...
              for (const reqDependencies in module.config.dependencies) {
                ...
                try {
                  if (!global.nodemodule.hasOwnProperty(reqDependencies)) {
                    global.nodemodule[reqDependencies] = requireDependency(reqDependencies, reqDependenciesPath);
                  } else '';
                } catch {
                  var check = false;
                  var isError;
                  ...
                  for (let i = 1; i <= 3; i++) {
                    try {
                      require['cache'] = {};
                      if (listPackage.hasOwnProperty(reqDependencies) || listbuiltinModules.includes(reqDependencies)) global['nodemodule'][reqDependencies] = requireDependency(reqDependencies, reqDependenciesPath);
                      else global['nodemodule'][reqDependencies] = requireDependency(reqDependencies, reqDependenciesPath);
                      check = true;
                      break;
                    } catch (error) {
                      isError = error;
              ...
              for (const dependency in event.config.dependencies) {
                ...
                try {
                  if (!global.nodemodule.hasOwnProperty(dependency)) {
                    global.nodemodule[dependency] = requireDependency(dependency, _0x21abed);
                  } else '';
                } catch {
                  let check = false;
                  let isError;
                  ...
                  for (let i = 1; i <= 3; i++) {
                    try {
                      ...
                      if (listPackage.hasOwnProperty(dependency) || listbuiltinModules.includes(dependency)) global.nodemodule[dependency] = requireDependency(dependency, _0x21abed);
                      else global.nodemodule[dependency] = requireDependency(dependency, _0x21abed);
                      check = true;
                      break;
                    } catch (error) {
                      isError = error;
...
process.on('unhandledRejection', (err) => {
  if (err == null) return;
  if (typeof err === 'object' && !Array.isArray(err) && Object.keys(err).length === 0) return;
  const msg = err.message || err.error || err.code || String(err);
  if (/ENOTFOUND|ECONNREFUSED|404|status code 404|status code 400|status code 429/i.test(msg)) return;
  if (err.code === 'ENOENT' && err.path && /totalChat\.json|noitu\.txt/i.test(err.path)) return;
  logger(msg.length > 300 ? msg.slice(0, 300) + '...' : msg, '[ UNHANDLED REJECTION ]');
})
